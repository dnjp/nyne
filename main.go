...
var eslint = flag.Bool("f", false, "run eslint on the entire file after Put")
...
func reformat(id int, name string) {
	...
	new, err := exec.Command("eslint", "--fix", name).CombinedOutput()
	if err != nil {
		if strings.Contains(string(new), "fatal error") {
			fmt.Fprintf(os.Stderr, "eslint %s: %v\n%s", name, err, new)
		} else {
